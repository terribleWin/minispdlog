...
void test_level_conversion() {
    ...
    // 测试所有级别
    level levels[] = {level::trace, level::debug, level::info, level::warn, level::error, level::critical, level::off};

    for (auto lvl : levels) {
        std::cout << "Level: " << std::setw(10) << level_to_string(lvl) << " | Short: " << level_to_short_string(lvl)
...
void test_string_to_level() {
    ...
    std::string test_strings[] = {"trace", "DEBUG", "Info", "WARN", "unknown"};

    for (const auto& str : test_strings) {
        auto lvl = string_to_level(str);
        std::cout << "Input: " << std::setw(10) << str << " -> " << level_to_string(lvl) << "\n";
    }
}
...
void test_should_log() {
    ...
    level test_levels[] = {level::trace, level::debug, level::info, level::warn, level::error};
