...
void benchmark_sync_st(int iterations) {
    ...
    results.push_back({"Spdlog - Sync ST", iterations, 1, elapsed, iterations / (elapsed / 1000.0)});
...
void benchmark_sync_mt(int iterations) {
    ...
    results.push_back({"Spdlog - Sync MT", iterations, 1, elapsed, iterations / (elapsed / 1000.0)});
...
void benchmark_async_mt(int iterations) {
    ...
    auto logger = spdlog::basic_logger_mt<spdlog::async_factory>("bench_async_block", "logs/spd_async_block.log");
    ...
    results.push_back({"Spdlog - Async Block", iterations, 1, call_time, iterations / (call_time / 1000.0)});
...
void benchmark_async_overrun(int iterations) {
    ...
    auto logger =
        spdlog::create_async_nb<spdlog::sinks::basic_file_sink_mt>("bench_async_overrun", "logs/spd_async_overrun.log");
    ...
    results.push_back({"Spdlog - Async Overrun", iterations, 1, call_time, iterations / (call_time / 1000.0)});
...
void benchmark_multi_thread_sync(int thread_count, int messages_per_thread) {
    ...
    results.push_back(
        {"Spdlog - Multi Sync MT", total_messages, thread_count, elapsed, total_messages / (elapsed / 1000.0)});
...
void benchmark_multi_thread_async(int thread_count, int messages_per_thread) {
    ...
    auto logger = spdlog::basic_logger_mt<spdlog::async_factory>("bench_multi_async", "logs/spd_multi_async.log");
    ...
    results.push_back(
        {"Spdlog - Multi Async MT", total_messages, thread_count, call_time, total_messages / (call_time / 1000.0)});
