...
void test_basic_rotation() {
    ...
    std::cout << "\n写入15条日志后的文件状态:\n";
    std::cout << "  " << base_filename << ": " << (file_exists(base_filename) ? "存在" : "不存在") << "\n";
...
void test_multiple_rotations() {
    ...
    for (size_t i = 0; i < max_files + 1; ++i) {
        ...
        if (i == 0) {
            std::cout << "  当前文件: " << filename << " - " << (exists ? " 存在" : " 不存在") << "\n";
        } else {
            std::cout << "  轮转文件 " << i << ": " << filename << " - " << (exists ? " 存在" : " 不存在") << "\n";
        }
    }
...
void test_max_files_limit() {
    ...
    std::cout << "max_files=" << max_files << " (最多保留 " << expected_total << " 个文件: 1个当前 + " << max_files
              << "个历史)\n\n";
