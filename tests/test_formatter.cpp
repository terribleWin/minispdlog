...
void test_pattern_compilation() {
    ...
    // 创建自定义 pattern
    std::string patterns[] = {"[%Y-%m-%d %H:%M:%S] [%l] %v", "%H:%M:%S.%L - %v", "[%L] [%n] [thread %t] %v",
                              "%Y年%m月%d日 %H时%M分%S秒 %v"};
...
void test_all_flags() {
    ...
    pattern_formatter formatter("Year:%Y Month:%m Day:%d Hour:%H Min:%M Sec:%S Level:%l(%L) Name:%n Thread:%t Msg:%v");
...
void test_level_formatting() {
    ...
    // 设置自定义 pattern
    sink->set_formatter(std::make_unique<pattern_formatter>("[%Y-%m-%d %H:%M:%S] [%L] %v"));

    level levels[] = {level::trace, level::debug, level::info, level::warn, level::error, level::critical};
...
void test_time_caching() {
    ...
    std::cout << "格式化 " << iterations << " 条日志耗时: " << duration.count() << " 微秒\n";
...
void test_formatter_in_sink() {
    ...
    // 为不同 sink 设置不同格式
    sink1->set_formatter(std::make_unique<pattern_formatter>("[简洁] [%H:%M:%S] %v"));

    sink2->set_formatter(std::make_unique<pattern_formatter>("[详细] [%Y-%m-%d %H:%M:%S] [%L] [%n] %v"));
...
void test_pattern_change() {
    ...
    // 修改 pattern
    sink->set_formatter(std::make_unique<pattern_formatter>("[%H:%M:%S] >>> %v <<<"));
...
void test_thread_id() {
    ...
    auto log_from_thread = [&formatter](int thread_num) {
        details::log_msg msg("ThreadTest", level::info, "Message from thread " + std::to_string(thread_num));
        fmt::memory_buffer buf;
        formatter.format(msg, buf);
        std::cout << std::string_view(buf.data(), buf.size());
    };
